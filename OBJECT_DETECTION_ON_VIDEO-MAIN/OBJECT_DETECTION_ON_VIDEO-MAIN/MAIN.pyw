'''ПРОГРАММА ,КОТОРАЯ ОПРЕДЕЛЯЕТ ОБЪЕКТЫ НА ВИДЕО и их считаем
ЗАПУСКАЕМ ПРОГРАММУ И УКАЗЫВАЕМ ПУТЬ К ВИДЕО ПОСЛЕ ПОЯВЛЕНИЯ ВОПРОСА :Path to video(or URL):
Result\input\example.gif
ПОСЛЕ ЭТОГО ПОЯВЛЯЕТСЯ ВТОРОЙ ВОПРОС: What we are looking for: -УКАЗЫВАЕМ ОБЪЕКТ ,КОТОРЫЙ МЫ ХОЧЕМ НАЙТИ НАПРИМЕР МАШИНЫ-car или люди -person(можно много через запятую)
и нажимаем ENTER,ПОСЛЕ ЭТОГО ПОЯВИТЬСЯ КАРТИНКА С РАСПОЗНАННЫМИ ОБЪЕКТАМИ И В ЛЕВОМ УГЛУ ПОЯВИТЬСЯ ИХ КОЛИЧЕСТВО  '''
import cv2
import numpy as np
#from art import tprint #я заблокировал так как блок art не работает
...
if __name__ == '__main__':
    ...
    # Logo
    #tprint("Object detection")#я заблокировал так как блок art не работает
    #tprint("by")#я заблокировал так как блок art не работает
    #tprint("paveldat")#я заблокировал так как блок art не работает
